...
/*        protected ResourceManager resourceManager = new ResourceManager();
            ...
            else
            {
                ...
                if (resource == null || resource.Message.ToLower() == "not found")
                {
                    if (createMetadataFromParadata)
                    {
                        ...
                        resource = CreateResourceFromParadata(docId, url, list, xdoc);
        ...
        protected Resource CreateResourceFromParadata(string docId, string url, XmlNodeList list, XmlDocument xdoc)
        {
            string status = "successful";
            Resource resource = new Resource();

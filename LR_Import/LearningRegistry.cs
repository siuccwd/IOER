...
namespace LearningRegistryCache2
{
    public class LearningRegistry : BaseDataController
    {
        ...
        public static int lrRecordsFailed = 0;
            ...
            if (passedEdits)
            {
                ...
                foreach (XmlNode node in schemaList)
                {
                    ...
                    else
                    {
                        ...
                        {
                            case "nsdl_dc":
                                if (nsdlHandler.NsdlMap(docID, url, payloadPlacement, xdoc))
                                    lrRecordsFailed++;
                                found = true;
                                break;
                            case "nsdl dc 1.02.020":
                            ...
                            case "hashtags":
                                ...
                                break;
                            case "lom":
                                if (lomHandler.LomMap(docID, url, payloadPlacement, xdoc))
                                    lrRecordsFailed++;
                                found = true;
                                break;
                            ...
                            case "ieee lom 2002":
                                ...
                                break;
                            case "lrmi":
                                if (lrmiHandler.LrmiMap(docID, url, payloadPlacement, xdoc) == false)
                                    lrRecordsFailed++;
        ...
        public void UndeleteResources()
        {
            ...
            if (ResourceManager.DoesDataSetHaveRows(ds))
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    string status = "successful";
                    searchManager.ImportRefreshResources(undelResIds);

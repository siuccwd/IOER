...
function doSearch() {
    ...
    doAjax("DoSearch", data, successDoSearch);
}
...
function doAjax(method, data, success) {
    $.ajax({
        url: "/Services/OrganizationService.asmx/" + method,
        async: true,
...
function loadResults(data, total) {
    ...
    for (i in data) {
        ...
        list.append(
          resultTemplate
            .replace(/{title}/g, current.name)
            .replace(/{orgID}/g, current.id)
            ...
            .replace(/{orgUrl}/g, current.url)
            .replace(/{city}/g, current.city)
        );

...
namespace ILPathways.Library
{
    ...
    public class BaseAppPage : System.Web.UI.Page
    {
        ...
        /// <summary>
        /// Get a request parameter - includes handling of the mcms query string the he format starting with NRMODE with the original url as part of NRORIGINALURL. If not found a blank will be returned
        /// </summary>
        /// <param name="parameter">Parameter to retrieve</param>
        /// <returns>parameter value from query string or a blank if parameter is not found</returns>
        public string GetMcmsRequestKeyValue( string parameter )
        {

            return GetMcmsRequestKeyValue( parameter, "" );
        ...
        /// <returns>parameter value from query string or the default value if parameter is not found</returns>
        public string GetMcmsRequestKeyValue( string parameter, string defaultValue )
        {
            //centralizing
            string request = FormHelper.GetRequestKeyValue( parameter, defaultValue );
            ...
            if ( request == "" )
            {
                ...
                if ( FormHelper.IsValidRequestString() == false )
                {
                    LogError( thisBaseClassName + ".GetMcmsRequestKeyValue: Suspect url was encountered" );
                    return defaultValue;
                }
                //check if within an MCMS url
                string pubUrl = this.GetPublicUrl( Server.UrlDecode( Request.QueryString.ToString() ) );

                int urlStartPos = pubUrl.ToLower().IndexOf( parameter.ToLower() );
                if ( urlStartPos > 0 )
                {
                    int urlEndPos = pubUrl.ToLower().IndexOf( "&", urlStartPos );

                    if ( urlEndPos > urlStartPos )
                    {
                        request = pubUrl.Substring( urlStartPos + ( parameter.Length + 1 ), urlEndPos - ( urlStartPos + ( parameter.Length + 1 ) ) );
                        request = Server.UrlDecode( request );
                    }
                    else
                    {
                        //assume to end of string but make sure a value is provided
                        if ( pubUrl.Length > ( urlStartPos + ( parameter.Length + 1 ) ) )
                        {
                            request = pubUrl.Substring( urlStartPos + ( parameter.Length + 1 ), pubUrl.Length - ( urlStartPos + ( parameter.Length + 1 ) ) );
                            request = Server.UrlDecode( request );
                        }
                        else
                        {
        ...
        /// <summary>
        /// Get a request parameter - includes handling of the mcms query string with the format starting with NRMODE with the original url as part of NRORIGINALURL. If not found the default value will be returned
        /// </summary>
        /// <param name="parameter"></param>
        /// <param name="defaultValue"></param>
        /// <returns></returns>
        public int GetMcmsRequestKeyValue( string parameter, int defaultValue )
        {
            //try standard method first
            int request = FormHelper.GetRequestKeyValue( parameter, defaultValue );
            if ( request > 0 )
            {
                return request;
            ...
            else
            {
                ...
                try
                {
                    ...
                    if ( urlStartPos > 0 )
                    {
                        ...
                        if ( urlEndPos > urlStartPos )
                        {
                            workRequest = pubUrl.Substring( urlStartPos + ( parameter.Length + 1 ), urlEndPos - ( urlStartPos + ( parameter.Length + 1 ) ) );
                            workRequest = Server.UrlDecode( workRequest );
                        }
                        else
                        {
                            //assume to end of string but make sure a value is provided
                            if ( pubUrl.Length > ( urlStartPos + ( parameter.Length + 1 ) ) )
                            {
                                workRequest = pubUrl.Substring( urlStartPos + ( parameter.Length + 1 ), pubUrl.Length - ( urlStartPos + ( parameter.Length + 1 ) ) );
                                workRequest = Server.UrlDecode( workRequest );
                            }
                        }
                        if ( workRequest.Length > 0 && IsInteger( workRequest ) )
                            request = Int32.Parse( workRequest );
                        else
                            request = defaultValue;
                    }
                    else

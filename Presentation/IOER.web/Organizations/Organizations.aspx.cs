...
namespace IOER.Organizations
{
	public partial class Organizations : BaseAppPage
	{
		...
		private void InitializeForm()
		{
			...
			RecordPrivileges = new Ibus.ApplicationRolePrivilege();
		...
		private void DoSearch(int selectedPageNbr, string sortTerm)
		{
			...
			try
			{
				List<Ibus.Organization> list = myManager.Organization_Search(filter, sortTerm, selectedPageNbr, pager2.PageSize, ref pTotalRows);
		...
		public void Get(int recId)
		{
			...
			{
				//get record
				Ibus.Organization entity = MyManager.EFGet(recId);
		...
		public void Get(string recId)
		{
			...
			{
				//get record
				Ibus.Organization entity = MyManager.EFGetByRowId(recId);
		...
		protected void ShowMember(int id)
		{
			...
			Ibus.OrganizationMember ombr = OrganizationBizService.OrganizationMember_Get(LastOrgId, user.Id);
			this.lblHistory.Text = ombr.HistoryTitle();

			this.SetListSelection(rblMemberType, ombr.OrgMemberTypeId.ToString());
			//get roles
			MyManager.OrganizationMember_FillRoles(ombr);

			foreach (Ibus.OrganizationMemberRole role in ombr.MemberRoles)
			{
				foreach (ListItem item in cblOrgRoles.Items)
				{
		...
		{
			Patron user = new Patron();
			Ibus.OrganizationMember ombr = new Ibus.OrganizationMember();
		...
		protected void UpdateOrgMember(Patron user, Ibus.OrganizationMember currentOmbr)
		{
			string statusMessage = "";
		...
		private void HandleOrgMbrRoles(Ibus.OrganizationMember orgMbr)
		{
			//get current roles
			//might be better to delete all and then add, otherwise we need a balance line
			MyManager.OrganizationMember_FillRoles(orgMbr);

			Ibus.OrganizationMemberRole role = new Ibus.OrganizationMemberRole();
			string statusMessage = "";

			//handle changes
			foreach (Ibus.OrganizationMemberRole role2 in orgMbr.MemberRoles)
			{
			...
			foreach (ListItem item in this.cblOrgRoles.Items)
			{
				...
				{
					bool isNew = true;
					foreach (Ibus.OrganizationMemberRole role3 in orgMbr.MemberRoles)
					{
					...
					if (isNew == true)
					{
						role = new Ibus.OrganizationMemberRole();
		...
		private void DoMembersSearch(int selectedPageNbr, string sortTerm)
		{
			...
			List<Ibus.OrganizationMember> list = MyManager.OrganizationMember_Search(filter, sortTerm, selectedPageNbr, membersPager2.PageSize, ref pTotalRows);
		...
		protected void DeleteRecord(int recordId)
		{
			try
			{
				string statusMessage = "";
				//get mbr
				Ibus.OrganizationMember mbr = MyManager.OrganizationMember_Get(recordId);

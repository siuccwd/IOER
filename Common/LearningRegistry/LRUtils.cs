...
namespace LearningRegistry
{
	...
	internal static class LRUtils
	{
        ...
        private static string LRUserId = ConfigurationManager.AppSettings["learningRegistryUserId"].ToString();
        private static string LRPassword = ConfigurationManager.AppSettings["learningRegistryPassword"].ToString();
		...
		internal static string HttpPostRequest(Uri baseUri, string action, string data, string contentType = "application/json", string username = "", string password = "")
		{
            ...
            req.UseDefaultCredentials = false;
            NetworkCredential creds = new NetworkCredential();
            creds.UserName = LRUserId;
            creds.Password = LRPassword;
            req.Credentials = creds;
		...
		{
			/*can have a maxlength error 2.5mb is max!
			...
			{
				return "System.InvalidOperationException: " + ioe.ToString();
			}
		}
	}

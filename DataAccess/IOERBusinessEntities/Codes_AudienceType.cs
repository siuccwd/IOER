//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace IOERBusinessEntities
{
    using System;
    using System.Collections.Generic;
    
    public partial class Codes_AudienceType
    {
        public Codes_AudienceType()
        {
            this.Resource_IntendedAudience = new HashSet<Resource_IntendedAudience>();
        }
    
        public int Id { get; set; }
        public string Title { get; set; }
        public string NsdtTitle { get; set; }
        public string Description { get; set; }
        public Nullable<bool> IsPathways { get; set; }
        public Nullable<bool> IsActive { get; set; }
        public Nullable<bool> IsPublishingRole { get; set; }
        public Nullable<int> WarehouseTotal { get; set; }
    
        public virtual ICollection<Resource_IntendedAudience> Resource_IntendedAudience { get; set; }
    }
}

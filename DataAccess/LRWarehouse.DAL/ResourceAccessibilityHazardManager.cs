...
namespace LRWarehouse.DAL
{
    public class ResourceAccessibilityHazardManager : BaseDataManager
    {
        ...
        public string ImportV2(MyEntity entity)
        {
            ...
            try
            {
                SqlHelper.ExecuteNonQuery(ConnString, CommandType.StoredProcedure, "[Resource.AccessibilityHazardImportV2]", parameters);
            }
            catch (Exception ex)
            {
                status = "ResourceAccessibilityHazardManager.ImportV2(): " + ex.Message;
                LogError("ResourceAccessibilityHazardManager.ImportV2(): " + ex.ToString());
            }
